...
const PUBLIC_ROUTES = [
  ...
  "/api/checkout",
  "/api/storefront",
  // Shoppers, not sellers. /api/products/{id} is what the mobile app reads a piece from, and
  // /api/products/availability is what the bag calls to grey out anything that sold while she was
  // deciding. Neither was listed, so both fell through to "session + pilot approval": the app got a
  // redirect to /login instead of JSON, and the bag's availability check failed into its own
  // .catch(() => {}) and quietly stopped marking pieces sold out. Both are read-only and public by
  // design; the routes themselves hide anything that isn't for sale.
  "/api/products",
  "/checkout",
  "/api/thread",
  "/thread",
