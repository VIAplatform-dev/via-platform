...
import { colors, spacing, radius, fonts, eyebrow } from "../../lib/portal-theme";
    ...
    <SellerScreen title="Notifications" back>
      ...
      {(["push", "email"] as const).map((group) => (
        <View key={group} style={{ marginTop: spacing.xl }}>
          <Text style={{ ...eyebrow, marginBottom: spacing.sm }}>{group.toUpperCase()}</Text>

          {group === "push" && push === "denied" ? (
            <Pressable
              onPress={() => void Linking.openSettings()}
              style={{ backgroundColor: colors.bgCard, borderWidth: 1, borderColor: colors.border, borderRadius: radius, padding: spacing.md, marginBottom: spacing.sm }}
            >
          ...
          {group === "push" && pushUnavailable ? (
            ...
            <View style={{ backgroundColor: colors.bgCard, borderWidth: 1, borderColor: colors.border, borderRadius: radius, padding: spacing.md }}>
