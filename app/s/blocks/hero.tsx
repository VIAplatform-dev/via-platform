...
function heroMedia(kit: EditKit) {
 ...
 return bm?.kind === "video"
  ...
  : p.image ? <img src={p.image} alt="" className="absolute inset-0 h-full w-full object-cover" /> : null;
}
...
function HeroBleed({ kit }: { kit: EditKit }) {
 ...
 return media ? (
  <div className="vya-hero-frame vya-fill relative w-full overflow-hidden" style={{ minHeight: "84vh" }}>
   ...
   <div
    className={`vya-hero-inner vya-free-canvas relative z-10 px-6 text-center text-white ${free ? "vya-hero-free" : "flex min-h-[84vh] flex-col items-center justify-end pb-24 pt-36"}`}
   ...
   >
    {moveGrip}
    <FreeField b={b} ctx={ctx} fieldKey="heading" tag="h2" value={p.heading} className="vya-heading max-w-3xl text-5xl leading-[1.04] @xl:text-7xl" style={{ fontFamily: head }} />
  ...
  <div className="vya-hero-inner vya-free-canvas relative px-6 py-32 text-center">
   <FreeField b={b} ctx={ctx} fieldKey="heading" tag="h2" value={p.heading} className="vya-heading mx-auto max-w-3xl text-5xl leading-[1.05] @xl:text-6xl" style={{ fontFamily: head }} />
   ...
   <div className="vya-slides-track flex h-full w-full snap-x snap-mandatory overflow-x-auto overflow-y-hidden">
    {slides.map((s, i) => (
     <div key={i} className="vya-slide relative h-full w-full shrink-0 snap-start snap-always" style={{ minHeight: "84vh" }}>
      ...
      <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.12) 0%, rgba(0,0,0,0.1) 45%, rgba(0,0,0,0.6) 100%)" }} />
      <div className="relative z-10 flex h-full flex-col items-center justify-end px-6 pb-24 pt-36 text-center text-white" style={{ minHeight: "84vh" }}>
       <h2 {...kit.txtItem(s.heading, (v) => set(i, "heading", v))} data-field={`slide${i}heading`} className="vya-heading max-w-3xl text-5xl leading-[1.04] @xl:text-7xl" style={{ fontFamily: head }} />
      ...
      <div className="absolute inset-x-0 bottom-7 z-10 flex justify-center gap-2">
       {slides.map((_, j) => (ctx.edit
        ...
        : <a key={j} href={`#slide-${j}`} aria-label={`Slide ${j + 1}`} className="h-1.5 w-1.5 rounded-full transition" style={{ background: j === i ? "#fff" : "rgba(255,255,255,0.45)" }} />
       ))}
      </div>
...
function HeroStack({ kit }: { kit: EditKit }) {
 ...
 return (
  <div className="vya-fill flex w-full flex-col">
   <div className="vya-hero-inner vya-free-canvas relative px-6 pb-14 pt-24 text-center @xl:pt-32">
    {moveGrip}
    <FreeField b={b} ctx={ctx} fieldKey="heading" tag="h2" value={p.heading} className="vya-heading mx-auto max-w-4xl text-5xl leading-[1.03] @xl:text-7xl" style={{ fontFamily: head }} />
...
function HeroFrame({ kit }: { kit: EditKit }) {
 ...
 return (
  <div className="vya-fill relative w-full px-5 py-10 @xl:px-14 @xl:py-16">
   <div className="vya-round relative w-full overflow-hidden" style={{ minHeight: "62vh", background: `${fg}0d` }}>
    {p.image && <img src={p.image} alt="" className="absolute inset-0 h-full w-full object-cover" />}
    {p.image && <div className="absolute inset-0" style={{ background: "linear-gradient(to bottom, rgba(0,0,0,0.05) 40%, rgba(0,0,0,0.55) 100%)" }} />}
    <div className={`vya-hero-inner vya-free-canvas relative flex flex-col items-start justify-end px-7 pb-12 pt-40 @xl:px-12 ${p.image ? "text-white" : ""}`} style={{ minHeight: "62vh" }}>
     {moveGrip}
     <FreeField b={b} ctx={ctx} fieldKey="heading" tag="h2" value={p.heading} className="vya-heading max-w-2xl text-4xl leading-[1.05] @xl:text-6xl" style={{ fontFamily: head }} />
