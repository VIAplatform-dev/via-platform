 ...
 */
export type EditedSection = { sec: number; html: string };
...
export type PageEdits = {
  ...
  *  written down. */
 imgStyles?: { id: number; style: string }[];
...
export function applyEdits(html: string, p: PageEdits): string {
 ...
 const ismap = new Map((p.imgStyles || []).map((e) => [e.id, sanitizeStyle(e.style)]));
 if (ismap.size) eachImage($, (el, id) => { const st = ismap.get(id); if (st) $(el).attr("style", mergeStyle($(el).attr("style") || "", st)); });
 ...
 const rebuilt = p.sections.map((entry) => {
  ...
  // Her own section, with whatever she has since put inside it.
  if ("sec" in entry) return secs[entry.sec] ? sanitizeCapturedSection(entry.html) : "";
