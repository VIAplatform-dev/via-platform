...
import { bagsSlugs, shoesSlugs, accessoriesSlugs, clothingSlugs } from "./categoryMap.ts";
...
const categoryKeywords: [CategorySlug, string[]][] = [
 ...
 // A charm hangs OFF a bag; it is neither a bag nor a wallet. First, so "Bag Charm &
 ...
 // not a wallet) keep winning as bags.
 ["wallets", [
 "wallet", "billfold", "bifold", "bi-fold", "trifold", "tri-fold",
 ...
 // "accessories" catch-all for watches, gloves, neckwear, and everything else
 ["accessories", [
 "watch",
 "keyring", "keychain",
 "gloves", "mittens",
...
export const OVERRIDE_FAMILY_TO_SLUG: Record<string, CategorySlug> = {
  bags: "bags",
  shoes: "shoes",
...
/** The top-level display family a slug belongs to, or null if it belongs to none. */
export function familyOf(slug: CategorySlug): string | null {
  if (bagsSlugs.has(slug)) return "bags";
  if (shoesSlugs.has(slug)) return "shoes";
  if (accessoriesSlugs.has(slug)) return "accessories";
  if (clothingSlugs.has(slug) || slug === "other-clothing") return "clothing";
...
export function categoryFor(title: string | null | undefined, overrideFamily?: string | null): CategorySlug {
  ...
  if (!overrideFamily) return inferred;
  const fallback = OVERRIDE_FAMILY_TO_SLUG[overrideFamily];
  if (!fallback) return inferred;
  return familyOf(inferred) === overrideFamily ? inferred : fallback;
}
