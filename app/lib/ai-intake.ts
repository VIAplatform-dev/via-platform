...
import { templateInstruction, type Template } from "./description-format";
import { layoutInstruction, type LayoutSection } from "./description-layout";
...
export async function draftListing(
 imageUrls: string[],
 voice?: { guide: string; examples: string[]; template?: Template | null; layout?: LayoutSection[] | null },
 hints?: string,
 ...
 // The examples are fenced, not quoted, and given room: a templated listing runs past 400
 // characters long before it reaches the measurements block, so the old cap showed the model the
 // top of the template and hid the rest of it.
 ...
 ? `\nReal examples of how they write their listings, reproduced exactly, line breaks included:\n${voice.examples.map((e, i) => `--- their listing ${i + 1} ---\n${e.slice(0, 1200)}`).join("\n\n")}`
 : "") +
 ...
 // HOW THE LISTING IS LAID OUT, and it OVERRIDES the house description rules it contradicts. Those
 // say two or three tight sentences and no cataloguing of seams and cuffs; a laid-out listing is a
 // hundred words that open by cataloguing exactly that. Handed both, a model splits the difference
 // and the layout comes back a paragraph.
 //
 // HERS BEATS THE ONE WE READ. The learned template can only repeat what she has already published,
 // so it is the wrong thing to consult the moment she wants her listings to read differently. Only
 // one of the two is ever sent: two descriptions of the same thing is the same blend problem again.
 // See description-layout.ts and description-format.ts.
 const templateBlock = voice?.layout?.length
 ? layoutInstruction(voice.layout, voice.template?.medianWords)
 : voice?.template ? templateInstruction(voice.template) : "";
 ...
 body: JSON.stringify({
 model: INTAKE_MODEL,
 // A templated shop's description runs to 200 words on top of every other field in the JSON.
 max_tokens: 2000,
 system: SYSTEM,
 messages: [{ role: "user", content: [...images, { type: "text", text: INSTRUCTION + knownBlock + voiceBlock + templateBlock + (hints || "") }] }],
 }),
 });
...
export async function polishDescriptionForSeo(
 ...
 voice?: { guide: string; examples: string[]; template?: Template | null; layout?: LayoutSection[] | null } | null,
): Promise<string> {
 const apiKey = process.env.ANTHROPIC_API_KEY;
 if (!apiKey) throw new Error("ANTHROPIC_API_KEY not set");
 ...
 const voiceBlock = voice?.guide
  ? `\n\nKeep it in THIS store's voice (don't make it sound like generic AI): ${voice.guide}` +
    (voice.examples?.length ? `\nExamples of how they write, line breaks included:\n${voice.examples.slice(0, 3).map((e, i) => `--- their listing ${i + 1} ---\n${e.slice(0, 1200)}`).join("\n\n")}` : "")
  : "";
 // A shop asking to "improve for search" wants the words better, not its layout gone.
 const tmpl = voice?.layout?.length
 ? layoutInstruction(voice.layout, voice.template?.medianWords)
 : voice?.template ? templateInstruction(voice.template) : "";
...
- Keep the seller's voice and any structure/template they used, exactly: the same labels, the same order, the same line breaks. Only tighten it to 2-3 sentences if their own listings are written that way.
...
Return ONLY the rewritten description text, no quotes, no preamble, no JSON.`;
 ...
 const res = await fetch(ANTHROPIC_URL, {
  ...
  body: JSON.stringify({
   model: INTAKE_MODEL,
   // A templated listing runs to 200 words; 400 tokens cut one off mid-measurement.
   max_tokens: 1200,
