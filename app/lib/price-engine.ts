...
import { resolveBrandLine, isFairComp } from "./brand-lines.ts";
...
export async function valueFromComps(
 ...
 // Which LINE of the house this is, so the model knows whether the comps in front
 // of it are the right market at all. A house name alone hides a 40x spread.
 const line = resolveBrandLine(ctx?.brand ?? null);
 const tierLine = line && line.label !== line.house
  ? `\n\nBRAND LINE: this is ${line.label}: the ${line.tier} line of ${line.house}, NOT the house in general. Comps from another tier of the same house (a diffusion or department-store line) are the WRONG market and have been filtered out already. If that leaves few or no true comps, say so and lower confidence. Do NOT fall back to the cheaper line's prices.`
  : "";
...
export { computePriceFlag } from "./price-flag-core.ts";
export type { PriceFlag } from "./price-flag-core.ts";
...
export async function getMarketReferenceFast(opts: { query: string; brand: string | null }): Promise<PriceEstimate | null> {
 ...
 // Same tier guard on the fast path. Grading a typed price against the wrong
 // line of the house is the same error, just quieter.
 const fastLine = resolveBrandLine(opts.brand ?? null);
 const beforeTier = pool.length;
 pool = pool.filter((c) => isFairComp(fastLine, c.title || ""));
 if (fastLine && pool.length !== beforeTier) console.log(`[pricing] grade dropped ${beforeTier - pool.length}/${beforeTier} cross-tier comps for ${fastLine.label}`);
...
export async function estimatePrice(opts: {
 ...
 // If nothing survives, price from the few that do rather than silently falling
 // back to the wrong market. ValueFromComps lowers confidence when comps are thin.
 const comps = sameTier.slice(0, 40);
