...
export async function runTool(slug: string, name: string, input: any): Promise<any> {
 ...
 // Carried, not rebuilt: this writes the whole brief back, so a field it forgets is a field the
 // assistant silently erases. Her chosen listing layout is one of them.
 await saveStoreBrief(slug, { pricing: input.brief.pricing ?? cur?.pricing ?? "", voice: input.brief.voice ?? cur?.voice ?? "", about: input.brief.about ?? cur?.about ?? "", layout: input.brief.layout ?? cur?.layout ?? [] });
 done.push("brief");
 }
