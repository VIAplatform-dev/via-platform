...
export async function POST(request: NextRequest) {
 ...
 // Same fix as the single-item checkout: per piece, what she measured, else what the piece IS.
 // The bare `|| 16` defaults sat exactly on the Small tier's limits, so an unmeasured bag of
 // pieces was sold with a small parcel's postage. See parcel-estimate.ts.
 parcel: (() => {
 const ps = reserved.map((it) => parcelFor(it));
 return {
  weightOz: ps.reduce((s, p) => s + p.weightOz, 0),
  lengthIn: Math.max(...ps.map((p) => p.lengthIn)),
  widthIn: Math.max(...ps.map((p) => p.widthIn)),
  heightIn: ps.reduce((s, p) => s + p.heightIn, 0),
 };
 })(),
 });
 ...
 // EVERY calculation behind a payment, including one that came to zero. This carried
 // `&& taxCents > 0`, so a sale where Stripe worked out that no tax was due filed nothing at all,
 // and the seller's reports had no record of the position having been taken. A zero is a finding,
 // not an absence. item-intent already did it this way.
