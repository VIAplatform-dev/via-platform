...
// The audience a request names: ?tag=a,b&spentOver=50&category=bags on GET, `audience` on POST.
function audienceFrom(q: URLSearchParams): AudienceFilter {
 return parseAudience({ tag: q.get("tag"), spentOver: q.get("spentOver"), category: q.get("category") });
}
...
export async function POST(request: NextRequest) {
 ...
 if (body.scheduledAt) {
  ...
  const c = await createScheduledCampaign(slug, { subject: common.subject, body: common.body, link: link || null, segment: encodeAudience(audience), scheduledAt: when });
