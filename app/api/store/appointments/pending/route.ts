...
import { countPendingAppointments, listPendingAppointments } from "@/app/lib/appointments/appointments-db";
...
export async function GET(request: NextRequest) {
 ...
 if (request.nextUrl.searchParams.get("list")) {
  const appointments = await listPendingAppointments(acting.seller.id);
